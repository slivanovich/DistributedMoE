...
def host_tte_instance():
    ...
    tte = NIXLTensorTransferEngine(config)
...
def test_write_transfer(host_tte_instance, remote_tte_instance, host_device, remote_device, dtype):
    ...
    src_tensor = torch.randn(128, 128, dtype=dtype).to(host_device)
    dst_tensor = torch.empty(128, 128, dtype=dtype).to(remote_device)
    ...
    assert host_tte_instance.transfer_engine is not None
    assert remote_tte_instance.transfer_engine is not None
    host_tte_instance.transfer_engine.add_remote_agent(remote_tte_instance.transfer_engine.get_agent_metadata())
    ...
    host_tte_instance.write_batch_transfer(
        ...
        src_data_sizes=[128 * 128 * dtype.itemsize],
    )
...
def test_read_transfer(host_tte_instance, remote_tte_instance, host_device, remote_device, dtype):
    ...
    assert host_tte_instance.transfer_engine is not None
    assert remote_tte_instance.transfer_engine is not None
    remote_tte_instance.transfer_engine.add_remote_agent(host_tte_instance.transfer_engine.get_agent_metadata())
